...
def apply_problem(problem: Problem, body: ProblemIn) -> None:
    problem.title = body.title; problem.description = body.description
    problem.input_description = body.input_description; problem.output_description = body.output_description
    problem.samples_json = json.dumps([x.model_dump() for x in body.samples], ensure_ascii=False)
    problem.constraints = body.constraints; problem.time_limit = body.time_limit
    problem.memory_limit = body.memory_limit; problem.difficulty = body.difficulty
...
async def get_problem(problem_id: str, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if not item: raise HTTPException(404, "problem not found")
...
async def create_problem(body: ProblemIn, _: User = Depends(teacher), db: Session = Depends(get_db)):
    if db.get(Problem, body.id): raise HTTPException(409, "problem id already exists")
    ...
    apply_problem(item, body); db.add(item); db.commit()
...
async def update_problem(problem_id: str, body: ProblemIn, _: User = Depends(teacher), db: Session = Depends(get_db)):
    if body.id != problem_id: raise HTTPException(400, "problem id cannot be changed")
    item = db.get(Problem, problem_id)
    if not item: raise HTTPException(404, "problem not found")
    apply_problem(item, body); db.commit()
