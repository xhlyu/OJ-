...
主要数据存储在 `data/oj.db`。SQLite 使用 WAL 和 busy timeout 改善并发读写。备份目录包含数据库文件和 `manifest.json`，清单记录文件名和 SHA-256。恢复前验证清单、摘要和 SQLite 完整性，并使用 SQLite 在线备份创建当前数据库的安全副本。替换数据库前清理 WAL/SHM 辅助文件，避免旧事务重新应用到恢复后的数据库；失败时不会破坏当前数据。
