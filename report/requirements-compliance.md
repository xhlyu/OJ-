...
结论范围：本项目以完成全部 **基础必做模块**、代码规范、自动化测试、实验报告和线下验收材料为目标。Adv 1 Special Judge、Adv 2 安全隔离、Adv 3 代码相似度检测均为选做，当前未实现，也不计入基础符合性结论。
...
| 模块 | 状态 | 说明 |
|---|---|---|
| Adv 1 Special Judge | 选做未实现 | 基础 standard 比较已完成 |
...
“完全符合”在此特指 **全部基础必做要求**。三个 Adv 模块是原文明示的选做内容，当前未实现；评测器也按原文基础模块边界不执行 memory_limit，且不宣称可安全公开部署。
